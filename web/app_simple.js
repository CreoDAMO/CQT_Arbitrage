...
/**
 * CryptoQuest Arbitrage Dashboard - Simple Working Version
 * Fixed navigation and section switching with complete functionality
 */
...
class CryptoQuestDashboard {
    ...
    async loadArbitrageData() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.updateArbitrageTable(data.opportunities || []);
            }
        } catch (error) {
    ...
    updateArbitrageTable(opportunities) {
        ...
        tableBody.innerHTML = opportunities.map(opp => `
            <tr>
                <td>
                    <span class="badge bg-primary">${opp.source}</span>
                    <i class="fas fa-arrow-right mx-2"></i>
                    <span class="badge bg-info">${opp.target}</span>
                </td>
                <td class="text-success fw-bold">${opp.profit}%</td>
                <td>
                    <div class="progress" style="height: 20px;">
                        <div class="progress-bar bg-success" style="width: ${opp.confidence * 100}%">
                            ${Math.round(opp.confidence * 100)}%
                        </div>
                    </div>
                </td>
                <td class="text-success fw-bold">$${opp.net_profit}</td>
                <td><span class="badge bg-success">${opp.status}</span></td>
                <td>
                    <button class="btn btn-sm btn-success" onclick="window.dashboard.executeArbitrage('${opp.id}')">
    ...
    executeArbitrage(opportunityId) {
        this.showNotification(`Executing arbitrage opportunity: ${opportunityId}`, 'info');
        this.speak(`Executing arbitrage opportunity ${opportunityId}`);
    }
    ...
    async startBot() {
        ...
        try {
            const response = await fetch(`${this.apiBaseUrl}/api/bot/start`, { method: 'POST' });
            if (response.ok) {
    ...
    async stopBot() {
        ...
        try {
            const response = await fetch(`${this.apiBaseUrl}/api/bot/pause`, { method: 'POST' });
            if (response.ok) {
